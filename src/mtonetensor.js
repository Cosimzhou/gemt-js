...
function MToneTenser(){
  this.tenser = [
      {counter:0, match:9, array:[], key:2741},//0:
...
MToneTenser.prototype.absorb = function (e){
  ...
  for (var t of this.tenser) {
    ...
    if (t.key & key) {
      // in tone
      if (t.match == 1) {t.array[t.array.length-1]++} else {t.match = 1;t.array.push(1)};
    } else {
      // out of tone
      if (t.match == 0) {t.array[t.array.length-1]++} else {t.match = 0;t.array.push(1)};
      t.counter++;
    }
  }
  this.total++;
...
MToneTenser.prototype.join = function (mtt) {
  ...
  for (var i = 0, t; i < 12; ++i) {
    var thv = this.tenser[i], mtv = mtt.tenser[i];
    var om = mtv.match;
