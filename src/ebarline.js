...
EBarline.prototype._forceReturn = function() {
...
EBarline.prototype._budget = function(ctx, x, trkPos) {
  ...
  switch (this._barlineType) {
    case 0: // single line
      ...
      epos.pushOperations(ctx._xmark(_, 0, { cut: 1 })._attach(line1, GStroke
        .Const.ConstraintX, 0));
      break;
    case 1: // end line
    ...
    case 2: // double line
      ...
      epos.pushOperations(line1 = ctx._Vline(x, oy, _)._attach(endTrk.bl,
        ...
        line2 = ctx._Vline(_, oy, _)._attach(line1, GStroke.Const
          .ConstraintX, 3)._attach(endTrk.bl, GStroke.Const.ConstraintY2),
        ctx._xmark(_, 3, { cut: 1 })._attach(line1, GStroke.Const
          .ConstraintX, 3));
      break;
    case 3: // repeat line begin
      ...
      for (var trk, i = 0; trk = trkPos[i]; ++i) {
        ...
        epos.pushOperations(ctx._dot(_, _, 2)._attach(line1, GStroke.Const
            .ConstraintX, 10)._attach(trk.bl, GStroke.Const.ConstraintY, -
            12),
          ctx._dot(_, _, 2)._attach(line1, GStroke.Const.ConstraintX, 10)
          ._attach(trk.bl, GStroke.Const.ConstraintY, -20));
      }

      epos.pushOperations(ctx._xmark(_, 3, {
        cut: epos.length(),
        eobj: this,
      })._attach(line1, GStroke.Const.ConstraintX, 3));
      break;
    case 4: // repeat line end
      ...
      for (var trk, i = 0; trk = trkPos[i]; ++i) {
        // no need attch for y
        epos.pushOperations(ctx._dot(_, _, 2)._attach(line1, GStroke.Const
            .ConstraintX, -8)._attach(trk.bl, GStroke.Const.ConstraintY, -
            12),
          ctx._dot(_, _, 2)._attach(line1, GStroke.Const.ConstraintX, -
            8)._attach(trk.bl, GStroke.Const.ConstraintY, -20));
      }
      epos.pushOperations(ctx._xmark(_, 3)._attach(line1, GStroke.Const
        .ConstraintX, 3));
      break;
    case 5: // dash line
      epos = new EPositionInfo(3, height, x, oy);
      for (var trk, i = 0; trk = trkPos[i]; ++i) {
        epos.pushOperations(line1 = ctx._vline(x, _, 7)._attach(trk.bl,
            GStroke.Const.ConstraintY, -32),
          ctx._vline(x, _, 7)._attach(trk.bl, GStroke.Const.ConstraintY, -
            19),
          ctx._vline(x, _, 7)._attach(trk.bl, GStroke.Const.ConstraintY, -8)
        );
      }
      break;
    case 6: // null line
    ...
    case 7: // repeat line end and another begin
      ...
      epos.pushOperations(
        line1 = ctx._Vline(x + 8, oy, y)._attach(endTrk.bl, GStroke.Const
          .ConstraintY2),
      ...
      for (var trk, i = 0; trk = trkPos[i]; ++i) {
        epos.pushOperations(ctx._dot(_, _, 2)._attach(
            line1, GStroke.Const.ConstraintX, -6)._attach(
            trk.bl, GStroke.Const.ConstraintY, -12),
          ctx._dot(_, _, 2)._attach(line1, GStroke
            .Const.ConstraintX, -6)._attach(trk.bl, GStroke.Const
            .ConstraintY, -20));
      }
      ...
      for (var trk, i = 0; trk = trkPos[i]; ++i) {
        epos.pushOperations(ctx._dot(_, _, 2)._attach(line1,
            GStroke.Const.ConstraintX, 12)._attach(trk.bl, GStroke.Const
            .ConstraintY, -12),
          ctx._dot(_, _, 2)._attach(line1, GStroke.Const
            .ConstraintX, 12)._attach(trk.bl, GStroke.Const
            .ConstraintY, -20));
      }
      ...
      epos.pushOperations(ctx._xmark(_, 0, {
        ...
        eobj: this,
      })._attach(line1, GStroke.Const.ConstraintX, 5));
      break;
  }
