    ...
    switch (this._barlineType) {
      case 0: // single line
        ...
        epos.pushOperations(
          (line1 = ctx
            ._Vline(x, oy, _)
            ._attach(endTrk.bl, GStrokeConstraintType.ConstraintY2))
        );
        epos.pushOperations(
          ctx
            ._xmark(_, 0, { cut: 1 })
            ._attach(line1, GStrokeConstraintType.ConstraintX, 0)
        );
        break;
      case 1: // end line
      ...
      case 3: // repeat line begin
        ...
        epos.pushOperations(
          ...
          ctx
            ._Vline(_, oy, _)
            ._attach(line1, GStrokeConstraintType.ConstraintX, 6)
            ._attach(endTrk.bl, GStrokeConstraintType.ConstraintY2)
        );
        for (let trk, i = 0; (trk = trkPos[i]); ++i) {
          ...
          epos.pushOperations(
            ctx
              ._dot(_, _, 2)
              ._attach(line1, GStrokeConstraintType.ConstraintX, 10)
              ._attach(trk.bl, GStrokeConstraintType.ConstraintY, -12),
            ctx
              ._dot(_, _, 2)
              ._attach(line1, GStrokeConstraintType.ConstraintX, 10)
      ...
      case 4: // repeat line end
        ...
        for (let trk, i = 0; (trk = trkPos[i]); ++i) {
          ...
          epos.pushOperations(
            ctx
              ._dot(_, _, 2)
              ._attach(line1, GStrokeConstraintType.ConstraintX, -8)
              ._attach(trk.bl, GStrokeConstraintType.ConstraintY, -12),
            ctx
              ._dot(_, _, 2)
              ._attach(line1, GStrokeConstraintType.ConstraintX, -8)
        ...
        epos.pushOperations(
          ctx._xmark(_, 3)._attach(line1, GStrokeConstraintType.ConstraintX, 3)
        );
        break;
      case 5: // dash line
        epos = new EPositionInfo(3, height, x, oy);
        for (let trk, i = 0; (trk = trkPos[i]); ++i) {
          epos.pushOperations(
            (line1 = ctx
              ._vline(x, _, 7)
              ._attach(trk.bl, GStrokeConstraintType.ConstraintY, -32)),
            ctx
              ._vline(x, _, 7)
              ._attach(trk.bl, GStrokeConstraintType.ConstraintY, -19),
            ctx
              ._vline(x, _, 7)
              ._attach(trk.bl, GStrokeConstraintType.ConstraintY, -8)
          );
        }
        break;
      case 6: // null line
      ...
      case 7: // repeat line end and another begin
        ...
        epos.pushOperations(
          ...
          ctx
            ._VlineW(_, oy, y, 3)
            ._attach(line1, GStrokeConstraintType.ConstraintX, 2)
            ._attach(endTrk.bl, GStrokeConstraintType.ConstraintY2)
        );
        ...
        epos.pushOperations(
          ctx
            ._Vline(_, oy, y)
            ._attach(line1, GStrokeConstraintType.ConstraintX, 7)
            ._attach(endTrk.bl, GStrokeConstraintType.ConstraintY2)
        );
        ...
        for (let trk, i = 0; (trk = trkPos[i]); ++i) {
          epos.pushOperations(
            ctx
              ._dot(_, _, 2)
              ._attach(line1, GStrokeConstraintType.ConstraintX, 12)
              ._attach(trk.bl, GStrokeConstraintType.ConstraintY, -12),
            ctx
              ._dot(_, _, 2)
              ._attach(line1, GStrokeConstraintType.ConstraintX, 12)
