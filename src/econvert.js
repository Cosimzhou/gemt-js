...
function EConvert(mscore) {
      ...
      for (var i = 0; i < bars.length; ++i) {
        ...
        if (!last_bars || !bars[i]._clef._equal(last_bars[i]._clef)) {
          ...
          score._tracks[i].clefMarks.push({
            marksIdx: score._tracks[i].marks.length,
            headerMarks: arr
          });
        }
        if ((i == 0) &&
        ...
        if (ch.beat._start == minBeat) {
          ...
          if (ch._beamCombine) {
            if (ch._beamCombine._beamPhase == 0) {
              ...
              newobj._beamCombine = {
                _originBeamComb: ch._beamCombine,
                _beamPhase: 0
              };
              ...
              var upd = 0,
                openobj = openBeamCombine[ch._beamCombine.id];
              openobj._eobjects.push(newobj);
              newobj._beamCombine = {
                _originBeamComb: openobj,
                _beamPhase: 2
  ...
  { // auto append end line
    var emarks = score._tracks[0].marks;
    var terminalLine = emarks[emarks.length - 1];
